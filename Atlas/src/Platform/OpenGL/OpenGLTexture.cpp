...
namespace Atlas {
		...
		: m_Path(path)
	{
		...
		{ 
			ATL_PROFILE_SCOPE("stbi_load - OpenGLTexture2D::OpenGLTexture2D(const std::string&)");
		...
		m_DataFormat = m_DataFormat;

		ATL_CORE_ASSERT(m_InternalFormat & m_DataFormat, "Format not supported!");
	...
	void OpenGLTexture2D::SetData(void* data, uint32_t size)
	{
		...
		ATL_CORE_ASSERT(size == m_Width * m_Height * bytePerPixel, "Data must be entire texture!");
		glTextureSubImage2D(m_RendererID, 0, 0, 0, m_Width, m_Height, m_DataFormat, GL_UNSIGNED_BYTE, data);
	}
